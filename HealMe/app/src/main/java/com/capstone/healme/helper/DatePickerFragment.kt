...
class DatePickerFragment : DialogFragment(), DatePickerDialog.OnDateSetListener {
    ...
    override fun onAttach(context: Context) {
        ...
        mListener = if (parentFragment is DialogDateListener) {
            parentFragment as DialogDateListener?
        } else {
            throw ClassCastException("Parent fragment must implement DialogDateListener")
        }
    }
    ...
    override fun onDateSet(view: DatePicker, year: Int, month: Int, dayOfMonth: Int) {
        mListener?.onDialogDateSet(tag, year, month + 1, dayOfMonth)
    }
